...
use JavaScript;
use Session;
use App\Model\Post; // Added to use the Post model directly
...
class ProfileController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // --- NEW, CORRECTED CODE ---
        // Start a direct query for this user's posts
        $postsQuery = \App\Model\Post::query()->where('user_id', $this->user->id);

        // Apply the 18+ filter based on the VIEWER's setting
        if (Auth::check()) {
            $viewer = Auth::user();
            if (!$viewer->show_adult_content) {
                $postsQuery->where('is_adult_content', false);
            }
            // If show_adult_content is true, we add no filter, so all posts are shown.
        } else {
            // Guests can never see adult content
            $postsQuery->where('is_adult_content', false);
        }
        
        // If a category filter (cosplay/anime) is active from a button, apply it
        if ($postsFilter) {
            $postsQuery->where('content_type', $postsFilter);
        }

        // Get the final, paginated results, ordered by newest first
        $posts = $postsQuery->latest()->paginate(10);
        $posts = $posts->appends($request->query()); // Keep filters when changing pages
